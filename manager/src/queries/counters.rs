...
pub fn query(loader: &Loader, report_path: &Path) {
    ...
    // loader.store_selected_scopes(selected_scopes.elements.vec_into_relation_vec());
    ...
    //     .iter()
    //     .map(|RE((build, _package, _version, _krate, _crate_hash, _edition))| *build)
    //     .collect();
    ...
    // let iter = loader.load_iter_selected_scopes()
    //     .filter(
    //         |RE((
    //             _build,
    //             _mir_body_def_path,
...
pub fn new_query(loader: &Loader, report_path: &Path) {
    ...
    loader.store_selected_thir_blocks(selected_thir_blocks.elements.vec_into_relation_vec());
    ...
    for RE((
        ...
        check_mode,
        span,
    )) in loader.load_iter_selected_thir_blocks().filter(
        |RE((_build, _thir_body_def_path, _parent, _block, safety, _check_mode, _span))| {
            *safety == types::ScopeSafety::ExplicitUnsafe
        },
    ) {
        unsafe_thir_blocks_relation.push(RE((
            build,
            thir_body_def_path,
            block,
            span_resolver.get_expansion_kind(span),
    ...
    let unsafe_thir_statements = || {
        ...
        thir_statements
            .flat_map(|RE((stmt, _block, closest_unsafe_block, index))| {
                unsafe_thir_blocks_relation
                    ...
                    .map(
                        move |RE((
                            ...
                            RE((*build, stmt, closest_unsafe_block, index, *check_mode))
                        },
                    )
            })
    };
    ...
    loader.store_functions_unsafe_thir_blocks(functions_unsafe_thir_blocks.vec_into_relation_vec());

    let abis = loader.load_abis();
    let trait_items = loader.load_trait_items();
    let trait_items: HashSet<_> = trait_items
        .iter()
        .map(|RE((_trait_id, def_path, _defaultness))| def_path)
        .collect();
    let selected_function_definitions = loader.load_selected_function_definitions();
    let selected_function_definitions_thir_counts = selected_function_definitions.iter().map(
        |&RE((build, item, def_path, module, visibility, unsafety, abi, _return_ty, uses_unsafe))| {
            (
                build,
                def_path_resolver.resolve(def_path),
                item,
